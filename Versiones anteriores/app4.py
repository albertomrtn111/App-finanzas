...
def get_initial_balances():
    conn = get_connection()
    df = pd.read_sql(
        "SELECT id, account_name, amount, as_of_date, notes FROM initial_balances ORDER BY as_of_date, id",
        conn,
    )
    conn.close()
    return df

def insert_cash_snapshot(date_val, account, current_value, notes=None):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        """
        INSERT INTO cash_snapshots (date, account, current_value, notes, created_at)
        VALUES (%s, %s, %s, %s, %s)
        """,
        (date_val, account, float(current_value), notes if notes else None, datetime.now()),
    )
    conn.commit()
    cursor.close()
    conn.close()
...
def get_cash_df():
    conn = get_connection()
    df = pd.read_sql("SELECT * FROM cash_snapshots", conn)
    conn.close()
    if not df.empty:
        df["date"] = pd.to_datetime(df["date"])
    return df
...
def delete_cash_snapshot(cash_id: int):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM cash_snapshots WHERE id = %s", (cash_id,))
    conn.commit()
    cursor.close()
    conn.close()
            ...
            if not months:
                ...
                if df_income_year.empty and df_expenses_year.empty:
                    ...
                    if data:
                        ...
                        # Línea de ahorro por mes (ORDENADA ENERO->DICIEMBRE)
...
# ==============================================================================================================================
    ...
    with tab_g:
        ...
        if file_gastos is not None:
            try:
                # Leer con separador ';' (Excel en español)
                df_g = pd.read_csv(
                    file_gastos,
                    sep=";",          # CLAVE para tu Gastos.csv
                    encoding="utf-8",  # si lo has guardado así
                )

                # Limpiar nombres de columnas
                df_g.columns = [c.strip() for c in df_g.columns]

                # Normalizar nombre de categoría para evitar duplicados tipo 'comida'/'Comida'
                ...
                if missing:
                    ...
                    # Limpiar filas con fecha inválida
                    before = len(df_g)
                    ...
                    if before != after:
                        st.warning(
                            f"Se han descartado {before - after} filas por tener fechas no válidas."
                        )

                    # Asegurar numérico en amount
                    ...
                    if before != after2:
                        st.warning(
                            f"Se han descartado {before - after2} filas por tener importes no válidos."
                        )

                    # Botón para importar
                    if not df_g.empty:
                        st.markdown(
                            f"Se van a importar **{len(df_g)}** gastos si confirmas."
                        )
                        ...
                        if st.button("Importar gastos", key="btn_import_gastos"):
                            ...
                            cursor = conn.cursor()
                            try:
                                # Insertar categorías nuevas si no existen
                                existing_cats = set(
                                    get_expense_categories()["name"].tolist()
                                )
                                new_cats = set(df_g["category"].dropna().unique()) - existing_cats
                                for cat in new_cats:
                                    if isinstance(cat, str) and cat.strip():
                                        # cat ya viene normalizada (Capitalizada)
                                        add_expense_category(cat.strip())

                                # Insertar gastos
                                query = """
                                    INSERT INTO expenses
                                    (date, amount, category, subcategory, payment_method, expense_type, notes, created_at)
                                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                                """
                                now = datetime.now()
                                rows = []
    ...
    with tab_i:
        ...
        if file_ingresos is not None:
            try:
                # Mismo criterio: separador ';' y coma decimal si la hubiera
                df_i = pd.read_csv(
                    file_ingresos,
                    sep=";",
                    encoding="utf-8",
                ...
                # Normalizar categoría de ingresos
                ...
                if missing_i:
                    ...
                    if before_i != after_i:
                        st.warning(
                            f"Se han descartado {before_i - after_i} filas por tener fechas no válidas."
                        )
                    ...
                    after_i2 = len(df_i)
                    if before_i2 != after_i2:
                        st.warning(
                            f"Se han descartado {before_i2 - after_i2} filas por tener importes no válidos."
                        )

                    if not df_i.empty:
                        st.markdown(
                            f"Se van a importar **{len(df_i)}** ingresos si confirmas."
                        )
                        ...
                        if st.button("Importar ingresos", key="btn_import_ingresos"):
                            ...
                            try:
                                # Insertar categorías nuevas si no existen
                                existing_inc_cats = set(
                                    get_income_categories()["name"].tolist()
                                )
                                new_inc_cats = set(df_i["category"].dropna().unique()) - existing_inc_cats
                                for cat in new_inc_cats:
                                    if isinstance(cat, str) and cat.strip():
                                        add_income_category(cat.strip())

                                query_i = """
                                    INSERT INTO income
                                    (date, amount, source, category, notes, created_at)
                                    VALUES (%s, %s, %s, %s, %s, %s)
                                """
                                now = datetime.now()
                                rows_i = []
...
# ========================================================================================================================================================================================
    ...
    if submitted:
        if not account.strip():
            ...
            try:
                insert_cash_snapshot(fecha, account.strip(), amount, notes.strip() or None)
    ...
    df_last = get_last_cash(limit=30)
    if df_last.empty:
        ...
        with st.expander("Eliminar un snapshot"):
            ...
            if st.button("Eliminar snapshot seleccionado"):
                delete_cash_snapshot(options[sel])
...
# ========================================================================================================================================================================================
    ...
    if df_inv.empty and df_cash.empty:
        ...
        # ====== FOTO ACTUAL (último snapshot disponible) ======
        ...
        # ====== YTD % (comparando primer mes vs último mes disponible) ======
        month_names = {
            1: "Enero", 2: "Febrero", 3: "Marzo", 4: "Abril",
            5: "Mayo", 6: "Junio", 7: "Julio", 8: "Agosto",
            9: "Septiembre", 10: "Octubre", 11: "Noviembre", 12: "Diciembre",
        ...
        # ===== Serie temporal patrimonio (mensual) =====
